"""
Cross-service schema parity — now a canary for the zero-duplicate end state.

Every `CREATE TABLE` in this repo is `IF NOT EXISTS`, and for a long time
20-odd tables were declared by more than one service, because the schema was
defined inline in each service that touched a table rather than in a central
migration. That meant **whichever service starts first wins**: if two
declarations of the same table drifted apart, the schema you got depended on
container start order, and the loser's columns silently didn't exist. The
failure surfaced far from the cause — an `UndefinedColumnError` in an
unrelated query, only in deployments where the start order happened to differ.

That is over. `packages/schemas-py/dunetrace_schemas/migrations.py` is the
single declaration of every table more than one service touches
(migrations 2-12), and `scripts/check_schema_owners.py` fails CI on any table
with two owners or any `ADD COLUMN` on a table the file does not declare. What
this test adds on top is the *diagnosis*: it parses every
`CREATE TABLE IF NOT EXISTS` out of the service sources, asserts there are no
duplicates left, and — should one ever come back — reports the column-set and
column-type differences between the copies rather than just the count. The two
column-parity tests are parametrized over whatever duplicates exist, so they
skip (not error) in the expected state and light up with a real diff on a
regression.

Deliberately excluded: `ALTER TABLE`. `check_schema_owners.py` covers those
(a stray column is a second owner one column at a time); this file only reads
`CREATE TABLE` bodies. It also deliberately does not scan `migrations.py`:
that file is *supposed* to declare tables the services no longer do.

Run: make test-schema-parity
"""

from __future__ import annotations

import re
from collections import defaultdict
from pathlib import Path

import pytest

REPO_ROOT = Path(__file__).resolve().parent.parent
SERVICES_DIR = REPO_ROOT / "services"

# Captures the body between the outermost parens of a CREATE TABLE IF NOT EXISTS.
# Terminated by a line-leading `)`, then anything up to the statement end.
#
# The `[^;"']*` tail is load-bearing, not slop: `events` is declared
# `... ) PARTITION BY RANGE (received_at);`, so requiring `)` to be followed
# immediately by the terminator makes that match fail, and a failed match doesn't
# stop there — the non-greedy body keeps growing to the *next* `);` in the file
# and swallows whatever table comes after `events`. That silently dropped
# `failure_signals` from the parse until the canary test below caught it.
_CREATE_TABLE_RE = re.compile(
    r"CREATE TABLE IF NOT EXISTS (\w+)\s*\((.*?)\n\s*\)[^;\"']*[;\"']",
    re.DOTALL,
)

# Table-level constraints — not columns, so they're skipped when building the
# per-column map. (Constraint parity is not asserted; these are position- and
# name-sensitive in ways that make textual comparison noisy.)
_CONSTRAINT_KEYWORDS = frozenset({"PRIMARY", "FOREIGN", "UNIQUE", "CHECK", "CONSTRAINT", "EXCLUDE"})


def _service_sources() -> list[Path]:
    return sorted(
        p
        for p in SERVICES_DIR.rglob("*.py")
        if "test" not in p.parts and "__pycache__" not in p.parts
    )


def _parse_columns(body: str) -> dict[str, str]:
    """Map column name -> normalized definition text (type + inline constraints)."""
    columns: dict[str, str] = {}
    for raw_line in body.split("\n"):
        line = re.sub(r"--.*$", "", raw_line).strip().rstrip(",").strip()
        if not line:
            continue
        tokens = line.split()
        if tokens[0].upper() in _CONSTRAINT_KEYWORDS:
            continue
        # Collapse whitespace and upper-case so formatting differences between
        # two declarations of the same column don't read as drift.
        columns[tokens[0]] = " ".join(tokens[1:]).upper()
    return columns


def _collect_declarations() -> dict[str, dict[Path, dict[str, str]]]:
    """table -> {source file -> {column -> definition}}."""
    declarations: dict[str, dict[Path, dict[str, str]]] = defaultdict(dict)
    for path in _service_sources():
        source = path.read_text()
        for match in _CREATE_TABLE_RE.finditer(source):
            table, body = match.group(1), match.group(2)
            columns = _parse_columns(body)
            if columns:
                declarations[table][path] = columns
    return declarations


DECLARATIONS = _collect_declarations()
DUPLICATED = {t: per for t, per in DECLARATIONS.items() if len(per) > 1}

# The column-parity tests below are parametrized over the duplicates. In the
# expected state that set is empty, and an empty parametrize list is a pytest
# *error* on collection ("got empty parameter set"), not a pass — so stand in
# one explicitly-skipped case that says why.
_DUPLICATED_CASES = sorted(DUPLICATED) or [
    pytest.param(
        "<none>",
        marks=pytest.mark.skip(
            reason="no table is declared by more than one service — the end state"
        ),
    )
]


def _rel(path: Path) -> str:
    return str(path.relative_to(REPO_ROOT))


def test_parser_finds_the_known_declarations() -> None:
    """Guard against the regex silently matching nothing — which would make
    the zero-duplicates assertion below vacuously pass."""
    assert len(DECLARATIONS) > 20, (
        f"only parsed {len(DECLARATIONS)} CREATE TABLE statements; the regex has "
        "probably stopped matching the current formatting"
    )
    # events is partitioned and declared once, by ingest — a canary that we're
    # reading the real files. conversations is the detector's single-owner table.
    assert "events" in DECLARATIONS
    assert "conversations" in DECLARATIONS
    # The shared set lives only in packages/schemas-py/dunetrace_schemas/
    # migrations.py, which this test deliberately does not scan. If either of
    # these shows up here, a service has re-grown a copy.
    assert "failure_signals" not in DECLARATIONS
    assert "issues" not in DECLARATIONS


def test_no_table_is_declared_by_more_than_one_service() -> None:
    """The end state. A duplicate is a second owner, and a second owner is the
    whichever-starts-first contract that the migration runner replaced: a
    table two services touch belongs in migrations.py, not in either service."""
    rendered = "\n".join(
        f"  {table}: " + ", ".join(_rel(p) for p in sorted(per))
        for table, per in sorted(DUPLICATED.items())
    )
    assert not DUPLICATED, (
        f"{len(DUPLICATED)} table(s) are declared by more than one service:\n{rendered}\n"
        "Move the table into packages/schemas-py/dunetrace_schemas/migrations.py "
        "(a new, appended migration) and delete every service copy."
    )


@pytest.mark.parametrize("table", _DUPLICATED_CASES)
def test_duplicated_table_declares_the_same_columns(table: str) -> None:
    per_file = DUPLICATED[table]
    all_columns = set().union(*(set(c) for c in per_file.values()))

    problems = []
    for path, columns in sorted(per_file.items()):
        missing = all_columns - set(columns)
        if missing:
            problems.append(f"  {_rel(path)} is missing: {', '.join(sorted(missing))}")

    assert not problems, (
        f"`{table}` is declared in {len(per_file)} services with different column "
        f"sets. Because every CREATE TABLE is IF NOT EXISTS, the columns you get "
        f"depend on which service starts first.\n" + "\n".join(problems)
    )


@pytest.mark.parametrize("table", _DUPLICATED_CASES)
def test_duplicated_table_declares_the_same_column_types(table: str) -> None:
    per_file = DUPLICATED[table]
    files = sorted(per_file)

    problems = []
    for column in sorted(set().union(*(set(c) for c in per_file.values()))):
        definitions = {f: per_file[f].get(column) for f in files}
        present = {f: d for f, d in definitions.items() if d is not None}
        if len({*present.values()}) > 1:
            rendered = "\n".join(f"      {_rel(f)}: {d!r}" for f, d in present.items())
            problems.append(f"  {column}:\n{rendered}")

    assert not problems, (
        f"`{table}` is declared in {len(per_file)} services with conflicting "
        f"column definitions. The first service to start wins, so this is a "
        f"start-order-dependent bug:\n" + "\n".join(problems)
    )
