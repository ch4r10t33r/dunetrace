"""
The startup guard runs at import of ingest_svc.config, so the refusal has to be
...
Run: PYTHONPATH=packages/sdk-py:packages/schemas-py:services/ingest \
       python -m pytest services/ingest/tests/test_deploy_guard.py -v
"""
...
_SERVICE_DIR = _TESTS_DIR.parent  # services/ingest
...
def _import_config(
    ...
    return subprocess.run(
        [sys.executable, "-c", "import ingest_svc.config"],
        cwd=cwd,
        env=child_env,
        capture_output=True,
        text=True,
...
def test_prod_with_dev_auth_refuses_to_import(tmp_path):
    ...
    assert "dunetrace/ingest: refusing to start" in result.stderr
