"""
Metrics and readiness for the integrations worker (hardening Phase 3.1/3.2).

This worker is opt-in and its /ready is what a compose healthcheck calls, so an
inverted boolean, a forgotten set_schema_version or a mislabelled counter would
ship green without these. Covers, for each metric, the path it claims to
measure; the three ways _ready_check must say NOT ready (DB unreachable, schema
too old, poll loop stale) and the one way it says ready; and that a
metrics-server bind failure is logged rather than raised into startup.
...
Sibling file: test_elevenlabs_worker_metrics.py does the same for the other
worker built from this image.

No DB, no credentials, no network beyond 127.0.0.1.

Run:
    PYTHONPATH=packages/schemas-py:services/integrations \
        python -m pytest services/integrations/tests/test_worker_metrics.py -v
"""
...
import integrations_svc.worker as worker  # import before patch() resolves module paths
...
BACKLOG = "dunetrace_integrations_backlog"
PROCESSED = "dunetrace_integrations_processed_total"
FAILURES = "dunetrace_integrations_failures_total"
EXTERNAL_SECONDS_COUNT = "dunetrace_integrations_external_call_seconds_count"
EXTERNAL_CALLS = "dunetrace_integrations_external_calls_total"
POLL_SECONDS_COUNT = "dunetrace_integrations_poll_seconds_count"
...
def _provider_cls(evaluations=(), error=None):
    """A stand-in provider class — _poll_one calls provider_cls(url, **creds),
    so this has to be callable, not just an object with the right methods."""
    instance = MagicMock()
    instance.fetch_new_evaluations = AsyncMock(
...
class TestPollOneMetrics(unittest.IsolatedAsyncioTestCase):
    async def test_successful_poll_counts_result_ok(self):
        ...
        with (
            patch("integrations_svc.worker.decrypt_credentials", return_value={"api_key": "k"}),
            patch.dict("integrations_svc.worker._PROVIDER_CLASSES", {"langfuse": _provider_cls()}),
            patch("integrations_svc.worker.record_poll_success", AsyncMock()),
        ):
            await worker._poll_one("langfuse", _integration())
    ...
    async def test_failing_poll_counts_result_error_and_a_contained_failure(self):
        ...
        with (
            ...
            patch.dict(
                ...
                {"langfuse": _provider_cls(error=RuntimeError("provider 500"))},
            ),
            patch("integrations_svc.worker.record_poll_success", AsyncMock()),
            patch("integrations_svc.worker.record_poll_failure", AsyncMock(return_value=state)),
        ):
            await worker._poll_one("langfuse", _integration())  # contained, must not raise
...
class TestExternalCallMetrics(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_successful_fetch_observes_ok_once_for_its_provider(self):
        m = self._meter("langfuse")
        out = await worker._observed_call("langfuse", AsyncMock(return_value=["ev"]), 0.0)
        self.assertEqual(out, ["ev"])
        self.assertEqual(m.delta(EXTERNAL_SECONDS_COUNT, {"provider": "langfuse"}), 1.0)
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "langfuse", "status": "ok"}), 1.0)
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "langfuse", "status": "error"}), 0.0)

    async def test_failed_fetch_observes_error_and_re_raises(self):
        m = self._meter("langsmith")
        with self.assertRaises(RuntimeError):
            await worker._observed_call(
                "langsmith", AsyncMock(side_effect=RuntimeError("429")), 0.0
            )
        # A call that failed still took time and still cost a slot.
        self.assertEqual(m.delta(EXTERNAL_SECONDS_COUNT, {"provider": "langsmith"}), 1.0)
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "langsmith", "status": "error"}), 1.0)
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "langsmith", "status": "ok"}), 0.0)

    async def test_each_provider_is_its_own_series(self):
        m = self._meter("braintrust")
        await worker._observed_call("braintrust", AsyncMock(return_value=[]), 0.0)
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "braintrust", "status": "ok"}), 1.0)
        self.assertEqual(m.delta(EXTERNAL_SECONDS_COUNT, {"provider": "braintrust"}), 1.0)
    ...
    async def test_missing_provider_is_labelled_unknown_not_none(self):
        ...
        await worker._observed_call("", AsyncMock(return_value=[]), 0.0)
    ...
    async def test_an_integration_poll_observes_its_provider_fetch(self):
        ...
        labelled with the provider actually polled."""
        m = self._meter("braintrust")
        with (
            patch("integrations_svc.worker.decrypt_credentials", return_value={"api_key": "k"}),
            patch.dict(
                "integrations_svc.worker._PROVIDER_CLASSES", {"braintrust": _provider_cls()}
            ),
            patch("integrations_svc.worker.record_poll_success", AsyncMock()),
        ):
            await worker._poll_one("braintrust", _integration())
        self.assertEqual(m.delta(EXTERNAL_CALLS, {"provider": "braintrust", "status": "ok"}), 1.0)
...
class TestPollOnceMetrics(unittest.IsolatedAsyncioTestCase):
    def _due(self, per_provider: dict):
        async def _fetch(provider_name):
            return [_integration(id=i) for i in range(per_provider.get(provider_name, 0))]

        return patch(
            "integrations_svc.worker.fetch_due_integrations", AsyncMock(side_effect=_fetch)
        )
    ...
    async def test_backlog_is_every_due_integration_across_providers(self):
        with (
            self._due({"langfuse": 2, "langsmith": 1, "braintrust": 3}),
            patch("integrations_svc.worker._poll_one", AsyncMock()),
        ):
            self.assertEqual(await worker.poll_once(), 6)
        self.assertEqual(_sample(BACKLOG), 6.0)

    async def test_backlog_drops_back_to_zero_on_the_next_empty_wake(self):
        with (
            self._due({"langfuse": 2}),
            patch("integrations_svc.worker._poll_one", AsyncMock()),
        ):
...
# ── The loop in run_worker: poll timing, cycle failures, schema gauge ─────────
...
class TestRunWorkerLoopMetrics(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run_loop(self, poll_side_effect):
        ...
        with (
            patch("integrations_svc.worker.init_pool", AsyncMock()),
            patch("integrations_svc.worker.ensure_integrations_schema", AsyncMock()),
            patch("integrations_svc.worker.close_pool", close_mock),
            patch("integrations_svc.worker.poll_once", AsyncMock(side_effect=poll_side_effect)),
        ):
    ...
    async def test_a_completed_cycle_is_timed_and_the_schema_gauge_is_set(self):
        ...
        self.assertEqual(
            _sample(SCHEMA_GAUGE, {"service": "integrations"}), float(CURRENT_SCHEMA_VERSION)
        )
        close_mock.assert_awaited_once()

    async def test_a_failing_cycle_counts_where_cycle_and_keeps_the_loop_alive(self):
        m = _Meter((FAILURES, {"where": "cycle"}), (POLL_SECONDS_COUNT, None))
        await self._run_loop([RuntimeError("db gone"), 0, asyncio.CancelledError()])
...
class TestDisabledWorkerRegistersNothing(unittest.IsolatedAsyncioTestCase):
    async def test_disabled_worker_never_starts_a_metrics_server(self):
        with (
            patch.object(worker.settings, "INTEGRATIONS_WORKER_ENABLED", False),
            patch("integrations_svc.worker.init_pool", AsyncMock()) as init_mock,
...
class TestReadyCheck(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_a_cycle_still_running_is_not_called_dead(self):
        """A wake that is still fanning out across several orgs' provider APIs
        outlives 3 × WAKE_INTERVAL routinely; reporting unhealthy then invites
        an orchestrator to restart the busiest worker mid-cycle."""
        with patch.object(worker.settings, "WAKE_INTERVAL", 0.001):
            _complete_cycle()
...
class TestMetricsServer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_worker_startup_survives_a_metrics_server_that_never_came_up(self):
        ...
        close_mock = AsyncMock()
        with (
            patch.object(worker.settings, "INTEGRATIONS_WORKER_ENABLED", True),
            ...
            patch("integrations_svc.worker.ensure_integrations_schema", AsyncMock()),
            patch("integrations_svc.worker.close_pool", close_mock),
            patch("integrations_svc.worker.poll_once", poll_mock),
        ):
    ...
    async def test_server_serves_metrics_ready_and_health(self):
        port = _free_port()
        metrics.register_standard("integrations", worker.settings.APP_VERSION)
        ...
        with patch.object(worker.settings, "WAKE_INTERVAL", 10):
            ...
            try:
                ...
                self.assertEqual(status, 200)
                text = body.decode()
                self.assertIn('dunetrace_build_info{service="integrations"', text)
                self.assertIn(
                    f'dunetrace_schema_version{{service="integrations"}} '
...
class TestConfig(unittest.TestCase):
    def test_metrics_port_default(self):
        self.assertEqual(
            Settings.INTEGRATIONS_METRICS_PORT, int(os.getenv("INTEGRATIONS_METRICS_PORT", "9104"))
        )

    def test_the_two_workers_in_this_image_do_not_share_a_port(self):
        """One image, two processes: a single METRICS_PORT would collide in a
        single-container deployment."""
        self.assertNotEqual(Settings.INTEGRATIONS_METRICS_PORT, Settings.ELEVENLABS_METRICS_PORT)
